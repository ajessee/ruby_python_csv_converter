...
class AddDates:
    ...
    # }
    def create_date_object(self, headers, new_column_headers_array, csv_objects_array):
        if 'Date' in headers:
            date_object = {}
    ...
    def add_column_headers(self, headers, new_column_headers_array):
    ...
    def set_date_attributes_in_csv_obj(self, csv_object, date_object, headers):
        if 'Date' in headers and hasattr(csv_object, 'name'):
            ...
            csv_object.start_date, csv_object.end_date, csv_object.date_ranges = self.create_date_strings(
                unique_dates_list)

    # test if row data for column is blank
    def test_for_blank_row_data(self, headers, row, header_name, index):
    ...
    def create_date_strings(self, array_of_dates):
        start_date = ""
        end_date = ""
        range_string = ""
    ...
    def matcher(self, args_array):
        ...
        if match_array == ['add_dates', 'read']:
            ...
            return_object = self.test_for_blank_row_data(
            ...
            # create date object
            return_object = self.create_date_object(
                command_args["headers"], new_column_headers_array, command_args["csv_objects_array"])
        elif match_array == ['add_dates', 'write']:
            self.set_date_attributes_in_csv_obj(
                command_args["csv_object"], command_args["date_object"], command_args["headers"])
